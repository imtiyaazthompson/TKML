        ...
        if token.get_type() == WidgetType.LABEL:
            ...
            if bg == - 1 and fg != -1:
                ...
                if not current_frame:
                    ...
                    setup = insert_statement(
                        f"widgets['{name}'] = tk.Label(master=widgets[\"{current_frame[0]}\"],text='{token.get_text()}',width={width},"
            ...
            if bg == - 1 and fg != -1:
                if not current_frame:
                    ...
                    setup = insert_statement(
                        f"widgets['{name}'] = tk.Button(master=widgets[\"{current_frame[0]}\"],text='{token.get_text()}',width={width},"
            ...
            if bg == - 1 and fg != -1:
                if not current_frame:
                    ...
                    setup = insert_statement(f"widgets['{name}'] = tk.Entry(master=widgets[\"{current_frame[0]}\"],fg=\"{fg}\","
                                             f"width={width})", setup)
            elif fg == -1 and bg != -1:
                if not current_frame:
                    setup = insert_statement(f"widgets['{name}'] = tk.Entry(bg=\"{bg}\",width={width})", setup)
                else:
                    setup = insert_statement(f"widgets['{name}'] = tk.Entry(master=widgets[\"{current_frame[0]}\"],bg=\"{bg}\","
                                             f"width={width})", setup)
            elif bg == -1 and fg == -1:
                if not current_frame:
                    setup = insert_statement(f"widgets['{name}'] = tk.Entry(width={width})", setup)
                else:
                    setup = insert_statement(f"widgets['{name}'] = tk.Entry(master=widgets[\"{current_frame[0]}\"],width={width})", setup)
            else:
                if not current_frame:
            ...
            if not current_frame:
                ...
                setup = insert_statement(f"widgets[\"{name}\"] = tk.Frame(master=widgets[\"{current_frame[0]}\"])", setup)
                current_frame.insert(0, name)
